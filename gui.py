...
class APP(wx.Frame):
    ...
    def deal_task(self, task):
        ...
        if task['actObjType'] == 'image':
            if task['action'] == 'move_click':
                # 点击图片
                target_pic = self.wait_pic(f"res/{task['image']['picName']}.png", task['image']['picConfidence'], 1,
                                           task['circleCount'],
                                           task['circleWaitTime'])
                ...
                if type_target == dict:
                    ...
                    click_res = self.circle_move_click_pic(
                        target_pic.left + target_pic.width * task['image']['picLeft'],
                        target_pic.top + target_pic.height * task['image']['picTop'], 1,
            ...
            if task['action'] == 'move_click':
                click_res = self.circle_move_click_pic(task['position']['x'], task['position']['y'], 1,
                                                       task['circleCount'], task['circleWaitTime'])
                return click_res
            else:
                return "未识别的类型"
    ...
    def circle_move_click_pic(self, x, y, click_num, max_num, wait_time):
        try:
            movePosition(x, y)
            time.sleep(2)
            self.is_human = False
        ...
        if click_num > max_num:
            ...
            click_res = self.circle_move_click_pic(x, y, click_num, max_num, wait_time)
            return click_res
